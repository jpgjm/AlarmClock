...
struct AlarmEditView: View {
    ...
    /// スヌーズを `.custom` にするか。既定はオフ。
    /// `AlarmService.customSnoozeKey` と同じキーを使う。
    @AppStorage(AlarmService.customSnoozeKey) private var useCustomSnooze: Bool = false
        ...
        NavigationStack {
            Form {
                ...
                Section("スヌーズ") {
                    ...
                    Text("有効にすると AlarmKit のアラート画面に「スヌーズ」ボタンが表示されます。押すと指定分後に鳴り直します。アプリは開かず、画面も点きません。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                if draft.snoozeEnabled {
                    Section("スヌーズの方式 (検証用)") {
                        Toggle(isOn: $useCustomSnooze) {
                            Text("Intent に任せる (.custom)")
                        }

                        if useCustomSnooze {
                            Label {
                                Text("素の LiveContainer ではアラームを止められなくなります。"
                                     + "Intent が解決されないため、誰も stop() を呼びません。"
                                     + "検証が済んだらオフに戻してください。")
                            } icon: {
                        ...
                        Text("オフ (既定) では AlarmKit が内部で再鳴動を管理します"
                             + " (.countdown)。アプリのコードは走らないので、"
                             + "どの環境でも確実に止まります。\n\n"
                             + "オンにすると Intent 自身が stop() を呼ぶ方式 (.custom) になります。"
                             + "直インストールとパッチ版 LiveContainer では動きますが、"
                             + "素の LiveContainer では Intent が解決されないため止まりません。"
                             + "この差を確認するための項目です。")
                            .font(.caption)
                            .foregroundStyle(.secondary)

                        Text("この設定はアプリ全体に適用され、変更後に登録し直したアラームから有効になります。")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }
