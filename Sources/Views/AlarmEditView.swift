...
struct AlarmEditView: View {
        ...
        NavigationStack {
            Form {
                ...
                Section("再生するフォルダ") {
                    ...
                    Text("「アラーム音」を「デフォルト」にしている時、指定時刻にフォルダから1曲ランダムで直接鳴ります (アプリを開くたびに翌回の曲がシャッフルされます)。「音楽で起きる」ボタンでも再生できます。対応: wav / aiff / caf (推奨) / mp3 / m4a / aac。flac は AlarmKit 非対応で除外されます。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    /// customSoundName == nil の時は「デフォルト」、そうでなければライブラリから
    /// displayName を引いて表示 (見つからない場合はファイル名で代用)。
