...
struct AlarmItem: Codable, Identifiable, Equatable {
    ...
    ///
    /// **v17 では未使用**。スヌーズを AlarmKit ネイティブ (.countdown) に戻したため、
    /// 一時アラームを自前で作る必要が無くなった。
    /// 案 B の検証結果次第で v16 の `.custom` 方式に戻す可能性があるため、
    /// メソッドと関連フィールド (isSnoozeInstance / snoozeSourceID) は残してある。
    ///
