...
struct AlarmItem: Codable, Identifiable, Equatable {
    ...
    // MARK: - Fingerprint (AlarmKit 再登録判定用)

    /// AlarmKit への登録内容が変わったかを判定するためのフィンガープリント文字列。
    /// AlarmService はこれを保持しておき、前回登録時と一致するかつ既に .scheduled 状態
    /// なら再登録をスキップする。
    ///
    /// 含めるフィールド: AlarmKit の Alarm.Schedule / AlarmPresentation.Alert に反映される要素のみ
    ///   - hour / minute / schedule / label
    /// 含めないフィールド: アプリ内 (RingingView / AudioPlayerService) だけで使う要素
    ///   - folderRelPath / musicLibraryItemIDs / volume / fadeInSeconds / snoozeEnabled / snoozeMinutes
    /// これらは変えても AlarmKit 側の登録内容は変わらないので、再登録の必要はない。
    var scheduleFingerprint: String {
        ...
        case .oneShotAt(let date):
            // 秒精度で識別
            s += "o:\(Int(date.timeIntervalSince1970))"
        }
        s += "|label:\(label)"
        return s
    }
