...
//  v17 で案 B の検証構成に変更:
//    - secondaryButton = スヌーズ、secondaryButtonBehavior = .countdown に戻す
//      (AlarmKit ネイティブの再鳴動を復活させる)
//    - countdownDuration.postAlert = snoozeMinutes × 60 秒
//    - **同時に** secondaryIntent = SnoozeAndOpenIntent も渡す
//      → `.countdown` の時に Intent が呼ばれるかどうかを実機で検証する
//    - snoozeEnabled が false の場合は secondaryButton なし (停止のみ)
//    - stopIntent は StopAndOpenIntent (押下時にアプリを開いて次回抽選) のまま維持
//    - syncSchedule(skipIDs:) で、カウントダウン中のアラームを再登録しないようにできる
...
final class AlarmService {
    ...
    /// - Parameters:
    ///   - items: アプリ側の全アラーム
    ///   - skipIDs: 再登録をスキップしたいアラーム ID の集合。
    ///     スヌーズのカウントダウンが進行中のアラームを渡すことで、
    ///     schedule() による再登録でカウントダウンが巻き戻る / 消えるのを防ぐ。
    ///     stop の対象からは外れないので、削除・無効化は従来どおり反映される。
    func syncSchedule(with items: [AlarmItem], skipIDs: Set<UUID> = []) async {
        ...
        for item in desiredEnabled {
            if skipIDs.contains(item.id) {
                debugPrint("[Sync] skip reschedule for \(item.id) (countdown in progress)")
                continue
            }
            do {
    ...
    ///   .countdown behavior + countdownDuration.postAlert に snoozeMinutes 分を設定する。
    ///   AlarmKit が自動で N 分後に再鳴動させる (アプリは関与しない)。
    ///
    ///   加えて secondaryIntent (SnoozeAndOpenIntent) も渡している。これは
    ///   「.countdown の時に Intent が呼ばれるか」を実機で検証するための仕込み。
    ///   呼ばれた場合はアプリが前面に出て、AlarmListView に検証バナーが表示される。
    func schedule(_ item: AlarmItem) async throws {
        ...
        //
        // v17 (案 B の検証):
        //   `.countdown` に戻して AlarmKit ネイティブの再鳴動を使いつつ、
        //   secondaryIntent も渡してみる。`.countdown` の時に Intent が呼ばれるか
        //   ドキュメントに記載が無いため、実機で確かめるのが目的。
        //     - 呼ばれる  → アプリが開き、検証バナーが表示される
        //     - 呼ばれない → アプリは開かず、v15 と同じ挙動になる
        ...
        case .random:
            if let preparedName = SoundLibraryService.shared.prepareAlarmSound(
                alarmID: item.id,
        ...
        let configuration = AlarmManager.AlarmConfiguration<AlarmClockMetadata>(
            // .countdown スヌーズの再鳴動間隔。スヌーズ無効時は nil。
