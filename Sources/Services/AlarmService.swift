...
//    - 再鳴動は AlarmAppState.applyPendingSnoozeIfNeeded() が
...
final class AlarmService {
    ...
    ///   押下するとアプリが前面に出て、AlarmAppState.applyPendingSnoozeIfNeeded() が
    ...
    func schedule(_ item: AlarmItem) async throws {
        ...
        // AlarmAppState.applyPendingSnoozeIfNeeded() が N 分後の一時アラームを作り直す。
        ...
        case .random:
            if let preparedName = SoundLibraryService.shared.prepareAlarmSound(
                ...
                EventLog.log(.sound, alarmID: item.id, message: "抽選結果: \(preparedName)")
            } else {
