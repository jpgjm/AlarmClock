...
final class AlarmService: @unchecked Sendable {
    ...
    func schedule(_ item: AlarmItem) async throws {
        ...
        case .random:
            if let prepared = SoundLibraryService.shared.prepareAlarmSound(
                alarmID: item.id,
                ...
                EventLog.log(.sound, alarmID: item.id,
                             message: "抽選: \(prepared.sourceName) (候補 \(prepared.candidateCount) 曲中、履歴により \(prepared.excludedCount) 曲を除外)")
            } else {
