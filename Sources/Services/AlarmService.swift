...
final class AlarmService: @unchecked Sendable {
    ...
    /// スヌーズを `.custom` にするかどうかの UserDefaults キー。
    ///
    /// 既定は **オフ** (= `.countdown`)。
    ///
    /// 【危険】`.custom` は Intent 自身が `stop()` を呼ぶ契約なので、
    /// Intent が解決されない環境 (素の LiveContainer) では
    /// **アラームを止められなくなります**。
    ///
    /// この問題を本家に報告するための再現手段として用意しているもので、
    /// 常用するものではありません。設定画面にも警告を出しています。
    static let customSnoozeKey = "UseCustomSnooze"

    /// スヌーズを `.custom` にするか。
    static var useCustomSnooze: Bool {
        UserDefaults.standard.bool(forKey: customSnoozeKey)
    }
    ...
    func schedule(_ item: AlarmItem) async throws {
        ...
        // 【v50】環境による分岐を全廃した。
        //
        //   v28〜v49 は `RuntimeEnvironment.canUseCustomAppIntents` で
        //   直インストールと LiveContainer を分けていた。動作を揃えるため、
        //   どちらでも同じ `LCGuestIntent` を渡す形に統一する。
        //
        //   同じ型が環境によって解決先を変える。
        //
        //     | 環境 | 解決先 |
        //     |---|---|
        //     | 直インストール | このアプリ自身 (自前のメタデータに登録済み) |
        //     | 素の LiveContainer | どこにも無い → 解決されない |
        //     | パッチ版 LiveContainer | LC の LCGuestIntent → dylib |
        //
        //   `persistentIdentifier` は installd 内でアプリごとに独立しているので、
        //   LiveContainer と AlarmClock の両方が同じ値を名乗っても衝突しない。
        //
        //   これにより分岐が消え、再現手順も「アプリは同じで LiveContainer だけ
        //   入れ替える」という単純な形になる。
        ...
        if item.snoozeEnabled {
            ...
            alertPresentation = AlarmPresentation.Alert(
                ...
                // 【v51】設定でスヌーズの挙動を切り替えられるようにした。
                //
                //   .countdown (既定)
                //     AlarmKit が内部で再鳴動を管理する。アプリのコードは
                //     一切走らないので、どの環境でも確実に止まる。
                //
                //   .custom
                //     Intent 自身が stop() を呼ぶ契約。解決されない環境では
                //     **アラームを止められなくなる**。
                //     パッチ版 LiveContainer の dylib から AlarmKit が
                //     count=1 で見えることを確認済みなので、そちらでは動く。
                //
                //   既定をオフにしているのは、素の LiveContainer で
                //   止められなくなるため。報告用の再現手段として置いている。
                secondaryButtonBehavior: Self.useCustomSnooze ? .custom : .countdown
            )
            // .custom では AlarmKit のカウントダウンを使わないので nil。
            countdownDuration = Self.useCustomSnooze
                ? nil
        ...
        let configuration = AlarmManager.AlarmConfiguration<AlarmClockMetadata>(
            ...
            // 次回の曲が再抽選される。
            //
            // 直インストールでは `LCGuestIntent.perform()` (このアプリ内) が走り、
            // パッチ版 LiveContainer では LC 側の同名 Intent が解決されて
            // dylib の `AlarmClockGuestPerform` が呼ばれる。
            // 素の LiveContainer では解決されず、何も起きない (= 再現したい問題)。
            stopIntent: LCGuestIntent(
                handler: guestPayload == nil ? "" : Self.guestHandlerSymbol,
                action: guestPayload == nil ? "noop" : "reshuffle",
                payload: guestPayload ?? ""
            ),
            // 【v51】.custom のときはスヌーズにも同じ Intent を渡す。
            //   ハンドラが action="snooze" を受け取り、stop() を呼んでから
            //   再鳴動を仕込む。.countdown のときは AlarmKit に任せるので nil。
            secondaryIntent: Self.useCustomSnooze
                ? LCGuestIntent(
                    handler: guestPayload == nil ? "" : Self.guestHandlerSymbol,
                    action: "snooze",
                    payload: Self.makeSnoozePayload(for: item, base: guestPayload)
                  )
                : nil as LCGuestIntent?,
            sound: soundConfig
        )
    ...
    /// ハンドラに渡す payload を組み立てる (v50)。
    ///
    /// **パスはすべて絶対パス**にしてある。v48 までは LiveContainer の
    /// ホームからの相対にしていたが、v50 で分岐を全廃したので相対にする理由が
    /// 無くなった。
    ///
    ///   - 直インストール: このアプリ自身のコンテナの絶対パス
    ///   - LiveContainer : ゲストの `NSHomeDirectory()` は
    ///     `<LC home>/Documents/Data/Application/<UUID>` という実在の絶対パス
    ///
    /// どちらも受け手のプロセスからそのまま到達できる。
    /// `RuntimeEnvironment.lcRelativePath` は不要になった。
    ///
    ...
    private static func makeGuestPayload(
        ...
        let target = RuntimeEnvironment.alarmKitSoundsDirectory
    ...
    /// スヌーズ用の payload。抽選用の内容に、停止と再鳴動に必要な情報を足す。
    ///
    /// `.custom` ではハンドラが自分で `AlarmManager.shared.stop(id:)` を
    /// 呼ばないとアラートが鳴り止まないため、アラーム ID と分数が要る。
    private static func makeSnoozePayload(for item: AlarmItem, base: String?) -> String {
        var spec: [String: Any] = [:]
        if let base,
        ...
        spec["alarmID"] = item.id.uuidString
        spec["snoozeMinutes"] = item.snoozeMinutes
        guard let bytes = try? JSONSerialization.data(withJSONObject: spec),
