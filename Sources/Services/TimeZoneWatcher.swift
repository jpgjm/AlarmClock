...
final class TimeZoneWatcher {
    ...
    nonisolated static func registerBackgroundTask() {
        // 【v28】LiveContainer 内では登録できない。
        //   BGTaskSchedulerPermittedIdentifiers はホスト (LiveContainer) の
        //   Info.plist が参照されるため、このアプリの識別子は含まれていない。
        //   register() は false を返し、submit() は notPermitted で throw する。
        //   起動時に無駄な失敗ログが出るだけなので、最初から呼ばない。
        //
        //   TZ 補正の 3 層のうち、層 2 (バックグラウンド再チェック) だけが無効になる。
        //   層 1 (起動時チェック) と層 3 (NSSystemTimeZoneDidChange 通知) は
        //   LiveContainer 内でも通常どおり動く。
        guard RuntimeEnvironment.canUseBackgroundTasks else {
            debugPrint("[TimeZoneWatcher] LiveContainer 内のため BGTask の登録をスキップ")
            return
        }
    ...
    func scheduleNextBackgroundRefresh() {
        // registerBackgroundTask() をスキップした環境では submit も通らない。
        guard RuntimeEnvironment.canUseBackgroundTasks else { return }
