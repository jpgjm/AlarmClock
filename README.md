...
1. **音楽ファイルの転送**: iPhone/iPad の「ファイル」アプリで「このデバイス内」→「Alarm Clock」フォルダに `.flac` / `.mp3` / `.aac` / `.wav` / `.m4a` を置く。AirDrop / LocalSend からも可。
2. **アラーム追加**: 右上「+」で新規作成。デフォルトは **明後日 07:00** (PDF の "あさってアラーム" 主題)。
3. **フォルダ選択**: 編集画面「再生するフォルダ」でサブフォルダを選ぶと、そこ配下 (再帰) からランダム再生。未選択なら全体。
